...
class TypeDetails extends Component<any, any> {
    ...
    getApiMenu (typeId: any) {
        getApiMenu({apiId: typeId}).then((res: any) => {
            ...
            if (res.data.bllParamType) {
                this.setState({
                    bllParamType: res.data.bllParamType,
                    bllParamTypeName: res.data.bllParamTypeName
                })
            } else {
                this.setState({
                    bllParamType: 0,
    ...
    render() {
        let {publicParam, apiId, projectId, businessParam, bllParamType, bllParamTypeName, requestAddress, menuRecord, expandListApi, responseParams, componentStatus, codeDetail, responseCode, errCode, expandListPub, expandListHttp, expandListParams, expandListResponse, expandListRequest, expandListSample, expandListError, titleName} = this.state;
        if (componentStatus) {
            return (
                        ...
                        <div className="ClaimList-wrap">
                            ...
                            {businessParam && businessParam.length > 0 ? <div className="ClaimList-box">
                                <div className="ClaimList-title" onClick={() => this.toggleForm('业务请求参数')}>
                                    ...
                                    <h2 className="business-req">业务请求参数{bllParamTypeName ? <span>({bllParamType ? bllParamTypeName: ''})</span>: ''}</h2>
                                </div>
                            ...
                            <div className="ClaimList-box">
                                ...
                                {expandListApi ? <div className="tab-wrap">
                                    <Link className="link-border" to={`/apiTest/${apiId}/${projectId}`}>API测试工具</Link>
