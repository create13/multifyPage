...
import prompt from '@/static/image/prompt.png'; 
    ...
    submitTest() {
        ...
        paramsArray.map((item:any, index:number) => {
            if (item.is_necessary === '1' && (item.default_value === '' || item.default_value === null || item.default_value.trim() === '')) {
                if (!warnSpace) {
                    message.info(`请填写必填字段${item.param_name}`);
                }
                warnSpace = true;
                return;
        ...
        if (passCheck && !warnSpace) {
            getGiveData(paramObj).then((res:any) => {
                if (res.data.data) {
                    this.setState({
                        paramObj: paramObj.parameters,
                        backData: res.data.data
                    });
                } else if (res.data) {
                    if (JSON.stringify(res.data).substring(0, 1) === '{') {
                        this.setState({
                            paramObj: paramObj.parameters,
                            backData: res.data
                        });
                                                ...
                                                <select onChange={(e) => {this.changeApiName(e)}}>
                                                    {apiData.apis[selectKey].map((item: any, index: number) => {
                                                        if (defaultOption === item.api_name) {
                                                            return (
                                                                <option key={index} selected value={`${item.api_id}&${index}#${item.api_name}`}>{item.api_desc}</option>
                                                            )
                                                        } else {
                                                            return (
                                                                <option key={index} value={`${item.api_id}&${index}#${item.api_name}`}>{item.api_desc}</option>
                                                            )
                                                        }
                                                    })}
                                                </select>
                                                        ...
                                                        <dl key={index}>
                                                            ...
                                                            <dd>
                                                                <span className="desc-img">
                                                                    ...
                                                                    <span title={item.param_desc}><img src={`../${prompt}`} /></span>
                                                                </span>
                                                            </dd>
                                                        </dl>
                                                        )
                                                ...
                                                <dd> 
                                                    <div className="panel">
                                                        ...
                                                        <div className="panel-content requestContent">
                                                            <ReactJson src={paramObj} name={false} displayDataTypes={false} />
                                            ...
                                            <dl>
                                                ...
                                                <dd> 
                                                    <div className="panel">
                                                        ...
                                                        <ReactJson src={backData || {}} name={null} displayDataTypes={false} />
                                                        </div>
                                                    </div>
                                                </dd>
                                            </dl>
